...
async function openLesson59(page:Page){
  ...
  const lesson59=page.getByRole('button',{name:/Открыть урок 59:/});const lesson60=page.locator('.course-lesson-grid > button').nth(59);const lesson61=page.locator('.course-lesson-grid > button').nth(60);
  await expect(lesson59).toBeEnabled();await expect(lesson59).toHaveClass(/is-interactive/);await expect(lesson60).toBeEnabled();await expect(lesson61).toBeDisabled();
