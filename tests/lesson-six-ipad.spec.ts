...
const answers:Record<string,Answer> = {
  ...
  'l6-build-order': { type:'order', values:['Отметить точку A','Совместить нулевую отметку линейки с A','На отметке 6 см поставить точку B','Соединить A и B по линейке'] },
  ...
  'l6-challenge': { type:'input', value:'7' },
};
...
test('lesson 6 completes every stage and produces full scores on iPad WebKit',async({page})=>{
...
test('lesson 6 keeps an answer after direct page navigation',async({page})=>{
  ...
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:6,stageIndex:9}})));
  const endpoints=page.locator('[data-stage-id="l6-endpoints"]');
  await expect(endpoints).toBeVisible();
  await endpoints.getByRole('button',{name:'M и N',exact:true}).click();

  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:6,stageIndex:10}})));
  await expect(page.locator('[data-stage-id="l6-whole"]')).toBeVisible();
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:6,stageIndex:9}})));

  await expect(page.locator('[data-stage-id="l6-endpoints"] .choice-grid button.selected')).toHaveText('M и N');
});
