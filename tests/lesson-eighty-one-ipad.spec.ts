...
async function openLesson81(page:Page){
  await page.goto('/');
...
async function jump(page:Page,stageIndex:number){
  await page.evaluate(stageIndex=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:81,stageIndex}})),stageIndex);
}
...
test('lesson 81 remains usable after lesson 82 unlock and preserves corrected answer semantics',async({page})=>{
  ...
  await expect(stage).toHaveAttribute('data-stage-id','l81-mission');
  expect(await noHorizontalOverflow(page)).toBeTruthy();

  await jump(page,2);
  await expect(stage).toHaveAttribute('data-stage-id','l81-oral2-count');
  const input=stage.locator('.inline-answer input');
  for(const wrong of ['6','10']){
    await input.fill(wrong);
    await stage.locator('.check-button').click();
    await expect(stage.locator('.instant-feedback.bad')).toBeVisible();
  }
  await expect(stage.getByRole('button',{name:'Дальше →',exact:true})).toBeEnabled();
  await input.fill('8');
  await stage.locator('.check-button').click();
  await expect(stage.locator('.instant-feedback.good')).toBeVisible();
  await input.fill('9');
  const edited=await page.evaluate(()=>JSON.parse(localStorage.getItem('mathnikita-lesson-81-progress-v1')??'null'));
  expect(edited?.results?.['l81-p1']).toBe(false);
  await expect(stage.getByRole('button',{name:'Дальше →',exact:true})).toBeEnabled();
  await input.fill('8');
  await stage.locator('.check-button').click();
  await expect(stage.locator('.instant-feedback.good')).toBeVisible();
  ...
  await jump(page,17);
  await expect(stage).toHaveAttribute('data-stage-id','l81-617-length');
  const lengthInput=stage.locator('.inline-answer input');
  await lengthInput.fill('1,0');
  await stage.locator('.check-button').click();
  await expect(stage.locator('.instant-feedback.bad')).toBeVisible();
  ...
  await jump(page,19);
  await expect(stage).toHaveAttribute('data-stage-id','l81-617-volume');
  const volumeInput=stage.locator('.inline-answer input');
  await volumeInput.fill('10.00');
  await stage.locator('.check-button').click();
  await expect(stage.locator('.instant-feedback.bad')).toBeVisible();
  const beforeReload=await page.evaluate(()=>JSON.parse(localStorage.getItem('mathnikita-lesson-81-progress-v1')??'null'));
  expect(beforeReload?.results?.['l81-p1']).toBe(true);

  await page.reload();
  await page.getByRole('button',{name:/Открыть урок 81:/}).click();
  await page.locator('.lesson-opening-start').click();
  await expect(page.locator('.lesson-runtime:not([hidden]) .interactive-stage')).toHaveAttribute('data-stage-id','l81-617-volume');
  await jump(page,35);
  await expect(page.locator('.lesson-runtime:not([hidden]) .interactive-stage')).toHaveAttribute('data-stage-id','l81-summary');
