...
async function openLesson37(page:Page){
  await page.goto('/');
  ...
  await expect(lesson37).toBeVisible();
...
test('lesson 37 teaches angle notation visually, persists progress and exposes one 20-task practice gate',async({page})=>{
  ...
  const practice1=page.locator('[data-stage-id="l37-practice1"]');
  await practice1.getByRole('button',{name:'B',exact:true}).click();
  ...
  const saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('mathnikita-lesson-37-progress-v1')??'null'));
  expect(saved?.stageIndex).toBe(3);
  expect(saved?.results?.['l37-p1']).toBe(true);

  await page.getByRole('button',{name:/Все уроки/}).click();
  await page.getByRole('button',{name:/Открыть урок 37:/}).click();
  await page.locator('.lesson-opening-start').click();
  await expect(page.locator('[data-stage-id="l37-practice2"]')).toBeVisible();
