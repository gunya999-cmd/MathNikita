...
const plans: LessonPlan[] = [
  {
    lessonNumber: 1,
    summaryStageId: 'summary',
    expectedStages: 21,
    expectedQuiz: '5/5',
    expectedPractice: '6/6',
    answers: {
      ...
      compare: { type: 'choice', value: '<' },
      numberline: { type: 'choice', value: '6' },
...
async function openLesson(page: Page, lessonNumber: number) {
  ...
  await expect(lessons).toHaveCount(5);
...
for (const plan of plans) {
  test(`lesson ${plan.lessonNumber}: complete walkthrough of every exercise`, async ({ page }) => {
    ...
    for (let step = 0; step < plan.expectedStages; step += 1) {
      ...
      const activity = stage.locator('.activity-area');
