...
test('lesson 95 sends clean Russian opening and every internal stage narration to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{
    ...
    await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-95-audit'});
  });
  ...
  await expect.poll(()=>payloads.get('lesson-95-opening')??'').not.toBe('');
  const opening=payloads.get('lesson-95-opening')??'';
  ...
  for(let index=0;index<50;index+=1){
    ...
    const narrationId=`lesson-95-stage-${stageId}`;
  ...
  expect(seen.size,'lesson 95 must expose all 29 internal stages').toBe(29);
  expect(violations.join('\n')).toBe('');
  const source704=payloads.get('lesson-95-stage-l95-practice-01')??'';
  expect(source704.toLocaleLowerCase('ru-RU')).toContain('леопольд');
  expect(source704).not.toContain('8/21');
  const source708=payloads.get('lesson-95-stage-l95-practice-02')??'';
  expect(source708.toLocaleLowerCase('ru-RU')).toContain('банан');
});
