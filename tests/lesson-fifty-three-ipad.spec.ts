...
async function openLesson53(page:Page){
  ...
  const lesson53=page.getByRole('button',{name:/Открыть урок 53:/});const lesson54=page.locator('.course-lesson-grid > button').nth(53);const lesson55=page.locator('.course-lesson-grid > button').nth(54);const lesson56=page.locator('.course-lesson-grid > button').nth(55);const lesson57=page.locator('.course-lesson-grid > button').nth(56);const lesson58=page.locator('.course-lesson-grid > button').nth(57);const lesson59=page.locator('.course-lesson-grid > button').nth(58);
  await expect(lesson53).toBeEnabled();await expect(lesson53).toHaveClass(/is-control-ready/);await expect(lesson54).toBeEnabled();await expect(lesson55).toBeEnabled();await expect(lesson56).toBeEnabled();await expect(lesson57).toBeEnabled();await expect(lesson58).toBeEnabled();await expect(lesson59).toBeDisabled();
