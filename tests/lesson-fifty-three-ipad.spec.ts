...
async function openLesson53(page:Page){
  ...
  const lesson53=page.getByRole('button',{name:/Открыть урок 53:/});const lesson54=page.locator('.course-lesson-grid > button').nth(53);const lesson55=page.locator('.course-lesson-grid > button').nth(54);
  await expect(lesson53).toBeEnabled();await expect(lesson53).toHaveClass(/is-control-ready/);await expect(lesson54).toBeEnabled();await expect(lesson55).toBeDisabled();
