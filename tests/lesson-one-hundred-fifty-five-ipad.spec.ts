...
test('lesson 155 remains usable on iPad without horizontal overflow',async({page})=>{
 await page.goto('/');
 ...
 await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:155,stageIndex:7}})));
 const stage=page.locator('.interactive-stage[data-stage-id="l155-practice-01"]');
 await expect(stage).toBeVisible();await expect(stage.locator('input')).toHaveCount(3);
 ...
 expect(sizes.scrollWidth-sizes.clientWidth).toBeLessThanOrEqual(2);expect(sizes.stageWidth).toBeLessThanOrEqual(820);
});
