...
const answers:Record<string,Answer>={
  'l13-diagnostic':{type:'input',value:'5'},
  'l13-practice1':{type:'input',value:'10'},
  'l13-practice2':{type:'input',value:'90'},
  'l13-coordinate':{type:'choice',value:'M (4)'},
  'l13-practice3':{type:'input',value:'7'},
  'l13-practice4':{type:'input',value:'11'},
  'l13-practice5':{type:'input',value:'7'},
  'l13-practice6':{type:'order',values:['Провести луч и обозначить начало O','Подписать под O число 0','Выбрать единичный отрезок OE','Откладывать равные отрезки вправо','Подписать координаты точек']},
  'l13-quiz1':{type:'input',value:'10'},
  'l13-quiz2':{type:'choice',value:'0'},
  'l13-quiz3':{type:'choice',value:'единичный отрезок'},
  'l13-quiz4':{type:'input',value:'15'},
  'l13-quiz5':{type:'input',value:'9'},
  'l13-challenge':{type:'input',value:'6'},
};
...
test('lesson 13 completes every exercise and produces full scores on iPad WebKit',async({page})=>{
  ...
  await expect(page.locator('[data-stage-id="l13-summary"]')).toBeVisible();expect(visited.size).toBe(22);
...
test('lesson 13 keeps an answer after direct page navigation',async({page})=>{
  await openLesson(page);await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:13,stageIndex:4}})));
  const stage=page.locator('[data-stage-id="l13-practice1"]');await expect(stage).toBeVisible();await stage.locator('.inline-answer input').fill('10');
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:13,stageIndex:5}})));
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:13,stageIndex:4}})));
  await expect(page.locator('[data-stage-id="l13-practice1"] .inline-answer input')).toHaveValue('10');
});
