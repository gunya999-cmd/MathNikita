...
const EXPECTED_SHA='288dee483624db99d7cf6e0856df7040d2daa55d';
const BASE='https://mathnikita.gunya999.workers.dev';

test('production serves exact lesson 38 release with cloud profile, angle mastery and responsive flow',async({page,request,browserName})=>{
  ...
  await page.getByLabel('Имя ученика').fill(`QA38-${suffix}`);
  const pins=page.locator('.account-pin-row input');
  await pins.nth(0).fill('3838');
  await pins.nth(1).fill('3838');
  ...
  await expect(page.getByText('Полностью готовы 38 интерактивных уроков.')).toBeVisible();
