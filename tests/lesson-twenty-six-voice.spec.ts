...
async function installVoiceAudit(page:Page){
  await page.addInitScript(()=>{
    ...
    URL.createObjectURL=(blob:Blob|MediaSource)=>{
      if(blob instanceof Blob){const id=blobNarrationIds.get(blob);if(id)return`blob:l26-voice/${encodeURIComponent(id)}/${++sequence}`}
    ...
    class MockAudio{
      ...
      play(){this.pause();audit.audioPlays+=1;const prefix='blob:l26-voice/';audit.playedIds.push(this.src.startsWith(prefix)?decodeURIComponent(this.src.slice(prefix.length).split('/')[0]):this.src);this.timer=window.setTimeout(()=>{this.timer=null;this.onended?.()},28);return Promise.resolve()}
    }
    Object.defineProperty(window,'Audio',{configurable:true,writable:true,value:MockAudio});
    (window as unknown as {__l26VoiceAudit:VoiceAudit}).__l26VoiceAudit=audit;
...
async function routeNarration(page:Page,requests:NarrationRequest[]){
  ...
  await page.route('**/api/narration',async route=>{requests.push(route.request().postDataJSON() as NarrationRequest);await new Promise(resolve=>setTimeout(resolve,8));await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-26-voice'})});
}
...
async function openLesson(page:Page){
  ...
  await page.getByRole('button',{name:/Открыть урок 26:/}).click();
  await page.locator('.lesson-opening-start').click();
  await expect(page.locator('[data-stage-id="l26-mission"]')).toBeVisible();
}
async function played(page:Page,id:string){return page.evaluate(id=>(window as unknown as {__l26VoiceAudit:VoiceAudit}).__l26VoiceAudit.playedIds.includes(id),id)}
async function playedCount(page:Page,id:string){return page.evaluate(id=>(window as unknown as {__l26VoiceAudit:VoiceAudit}).__l26VoiceAudit.playedIds.filter(item=>item===id).length,id)}
...
test('lesson 26 plays Sulafat on every main stage and Pythagoras in mandatory practice',async({page})=>{
  ...
  const total=24;const seen=new Set<string>();
  for(let stageIndex=0;stageIndex<total;stageIndex+=1){
    await page.evaluate(stageIndex=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:26,stageIndex}})),stageIndex);
    ...
    const expectedId=`lesson-26-stage-${stageId}`;
  ...
  for(const action of mentorActions){
    const expectedId=`mentor-practice-26-${taskId}-${action.suffix}`;const before=await playedCount(page,expectedId);
...
test('lesson 26 navigation interrupts stale narration before the next stage speaks',async({page})=>{
  const requests:NarrationRequest[]=[];await installVoiceAudit(page);await routeNarration(page,requests);await openLesson(page);
  const first='lesson-26-stage-l26-mission';await expect.poll(()=>played(page,first),{timeout:6_000}).toBeTruthy();
  const pausesBefore=await page.evaluate(()=>(window as unknown as {__l26VoiceAudit:VoiceAudit}).__l26VoiceAudit.pauses);
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:26,stageIndex:1}})));
  await expect(page.locator('[data-stage-id="l26-warmup"]')).toBeVisible();
  const second='lesson-26-stage-l26-warmup';await expect.poll(()=>played(page,second),{timeout:6_000}).toBeTruthy();
  const pausesAfter=await page.evaluate(()=>(window as unknown as {__l26VoiceAudit:VoiceAudit}).__l26VoiceAudit.pauses);
