...
async function dispatchStage(page:import('@playwright/test').Page,stageIndex:number){
  await page.evaluate(index=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:93,stageIndex:index}})),stageIndex);
}

test('lesson 93 sends clean Russian opening and every internal stage narration to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{
    ...
    await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-93-audit'});
  });
  ...
  await expect.poll(()=>payloads.get('lesson-93-opening')??'').not.toBe('');
  const opening=payloads.get('lesson-93-opening')??'';
  expect(speechProblems(opening),opening).toEqual([]);
  expect(opening.toLocaleLowerCase('ru-RU')).toContain('цел');
  ...
  for(let index=0;index<60;index+=1){
    ...
    const narrationId=`lesson-93-stage-${stageId}`;
  ...
  expect(seen.size,'lesson 93 must expose all 30 internal stages').toBe(30);
  expect(violations.join('\n')).toBe('');
  const sourcePayload=payloads.get('lesson-93-stage-l93-practice-01')??'';
  expect(sourcePayload).toContain('равны 90');
  expect(sourcePayload).not.toContain('810, 225, 405');
});