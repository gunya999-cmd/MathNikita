...
for(let lessonNumber=1;lessonNumber<=55;lessonNumber+=1){
  ...
  test(`lesson ${lessonNumber} has exactly twenty mandatory-practice tasks`,()=>{
    ...
    if(lessonNumber===55)expect(extendedPracticeSetResponseCount(practice),'Lesson 55 must keep the 50-response contract').toBe(50);
  });
}
