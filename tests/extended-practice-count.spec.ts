...
for(let lessonNumber=1;lessonNumber<=56;lessonNumber+=1){
  ...
  test(`lesson ${lessonNumber} has exactly twenty mandatory-practice tasks`,()=>{
    ...
    if(lessonNumber===55||lessonNumber===56)expect(extendedPracticeSetResponseCount(practice),`Lesson ${lessonNumber} must keep the 50-response contract`).toBe(50);
  });
}
