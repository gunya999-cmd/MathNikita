...
test('lesson 172 remains usable on iPad without horizontal overflow',async({page})=>{
  await page.goto('/');
  ...
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:172,stageIndex:6}})));
  const stage=page.locator('.interactive-stage[data-stage-id="l172-practice-01"]');
