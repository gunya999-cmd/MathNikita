...
async function dispatchStage(page:import('@playwright/test').Page,stageIndex:number){
  await page.evaluate(index=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:96,stageIndex:index}})),stageIndex);
}

test('lesson 96 sends clean Russian opening and every internal stage narration to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{
    ...
    await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-96-audit'});
  });
  ...
  await expect.poll(()=>payloads.get('lesson-96-opening')??'').not.toBe('');
  const opening=payloads.get('lesson-96-opening')??'';
  expect(speechProblems(opening),opening).toEqual([]);
  expect(opening.toLocaleLowerCase('ru-RU')).toContain('правильн');
  ...
  for(let index=0;index<50;index+=1){
    ...
    const narrationId=`lesson-96-stage-${stageId}`;
  ...
  expect(seen.size,'lesson 96 must expose all 29 internal stages').toBe(29);
  expect(violations.join('\n')).toBe('');
  const source719=payloads.get('lesson-96-stage-l96-practice-01')??'';
  expect(source719.toLocaleLowerCase('ru-RU')).toContain('знаменател');
  expect(source719).not.toContain('1/8');
  const source721=payloads.get('lesson-96-stage-l96-practice-02')??'';
  expect(source721.toLocaleLowerCase('ru-RU')).toContain('числител');
});
