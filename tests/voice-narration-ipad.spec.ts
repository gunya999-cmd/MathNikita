...
async function installSpeechAudit(page:Page){
  await page.addInitScript(()=>{
    ...
    const synthesis={
      ...
      cancel:()=>{audit.cancelCount+=1},pause:()=>undefined,resume:()=>undefined,
...
async function assertNaturalRussianSpeech(page:Page,previousCount=0){
  ...
  expect(entry.voiceURI).toBe('ru-enhanced');expect(entry.voiceName).toBe('Milena Enhanced');expect(entry.lang).toBe('ru-RU');expect(entry.rate).toBeCloseTo(.94,5);expect(entry.pitch).toBe(1);expect(entry.text).toMatch(/[А-Яа-яЁё]/);expect(entry.text).not.toMatch(/[§№]/);return log.length;
}
...
test('every ready lesson uses natural Russian narration without overlapping the mentor',async({page})=>{
  ...
  for(let lessonNumber=1;lessonNumber<=20;lessonNumber+=1){
    ...
    const counterText=specialIds?'':await page.locator('.lesson-runtime:not([hidden]) .stage-counter').textContent();
