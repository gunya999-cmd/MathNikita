...
async function dispatchStage(page:import('@playwright/test').Page,stageIndex:number){
  await page.evaluate(index=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:92,stageIndex:index}})),stageIndex);
}

test('lesson 92 sends clean Russian opening and every internal stage narration to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{
    ...
    await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-92-audit'});
  });
  ...
  await expect.poll(()=>payloads.get('lesson-92-opening')??'').not.toBe('');
  const opening=payloads.get('lesson-92-opening')??'';
  ...
  for(let index=0;index<60;index+=1){
    ...
    const narrationId=`lesson-92-stage-${stageId}`;
  ...
  expect(seen.size,'lesson 92 must expose all 30 internal stages').toBe(30);
  expect(violations.join('\n')).toBe('');
  const sourcePayload=payloads.get('lesson-92-stage-l92-practice-01')??'';
  expect(sourcePayload).toContain('числа 36');
  expect(sourcePayload).not.toContain('12, 27, 30, 16, 15');
});
