...
async function prepare(page: Page) {
  await page.route('**/api/narration', async route => {
    await route.fulfill({ status: 200, headers: { 'content-type': 'audio/wav', 'x-mathnikita-voice': 'Sulafat', 'x-mathnikita-provider': 'gemini-flash', 'x-mathnikita-narration': 'ru-teacher-gemini-sulafat-v2', 'x-mathnikita-cache': 'hit' }, body: 'RIFFmockWAVEaudio-data-for-lifecycle' });
  });
  await page.addInitScript(() => {
    ...
    const state: MockState = { played: [], pauses: [], playCalls: [] };
    Object.defineProperty(window, '__mathnikitaWebkitAudioAudit', { value: state, configurable: true });
...
async function state(page: Page): Promise<MockState> {
  return page.evaluate(() => (window as unknown as { __mathnikitaWebkitAudioAudit: MockState }).__mathnikitaWebkitAudioAudit);
}

async function waitPlayed(page: Page, lesson: number, afterCount: number) {
  await expect.poll(async () => (await state(page)).played.slice(afterCount).filter(item => item.lessonNumber === lesson).length, { timeout: 10_000 }).toBeGreaterThan(0);
  return (await state(page)).played.slice(afterCount).filter(item => item.lessonNumber === lesson).at(-1)!;
}
...
for (let lesson = 1; lesson <= 90; lesson += 1) {
  test(`lesson ${lesson} WebKit/iPad voice lifecycle: play, interrupt, switch, replay`, async ({ page }) => {
    ...
    // Opening narration can be started and stopped on WebKit.
    const narrator = page.locator('.voice-narrator > button').first();
    const openingPlayedBaseline = (await state(page)).played.length;
    await narrator.click();
    const openingPlayed = await waitPlayed(page, lesson, openingPlayedBaseline);
    expect(openingPlayed.narrationId).toBe(`lesson-${String(lesson).padStart(2, '0')}-opening`);
    const openingPauseBaseline = (await state(page)).pauses.length;
    await narrator.click();
    await expect.poll(async () => (await state(page)).pauses.length).toBeGreaterThan(openingPauseBaseline);
    ...
    const beforeFirst = (await state(page)).played.length;
    const first = await waitPlayed(page, lesson, beforeFirst);
    expect(first.narrationId).toMatch(new RegExp(`^lesson-${String(lesson).padStart(2, '0')}-stage-`));

    const beforeAdvance = await state(page);
    expect(await clickNext(runtime), `lesson ${lesson}: next control`).toBe(true);
    await expect.poll(async () => stage.getAttribute('data-stage-id')).not.toBe(firstStageId);
    ...
    const second = await waitPlayed(page, lesson, beforeAdvance.played.length);
    expect(second.narrationId).not.toBe(first.narrationId);
    const switched = await state(page);
    expect(switched.pauses.at(-1)!.at).toBeLessThanOrEqual(second.at);

    const pauseBaseline = switched.pauses.length;
    await narrator.click();
    await expect.poll(async () => (await state(page)).pauses.length).toBeGreaterThan(pauseBaseline);
    const replayBaseline = (await state(page)).played.length;
    await narrator.click();
    const replay = await waitPlayed(page, lesson, replayBaseline);
    expect(replay.narrationId).toBe(second.narrationId);
  });
}
