...
test('lesson 108 sends clean Russian opening and all 11 control stage payloads to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{const payload=(route.request().postDataJSON()??{}) as Payload;if(payload.id&&payload.text)payloads.set(payload.id,payload.text);await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-108-audit'})});
  await page.goto('/');await page.locator('button[aria-label^="Открыть урок 108:"]').evaluate((node:HTMLButtonElement)=>node.click());await page.locator('.voice-narrator > button').first().click();
  await expect.poll(()=>payloads.get('lesson-108-opening')??'').not.toBe('');const opening=payloads.get('lesson-108-opening')??'';expect(speechProblems(opening),opening).toEqual([]);
  ...
  for(let index=0;index<20;index+=1){await jump(page,index);if(index>0){try{await expect.poll(async()=>await active.getAttribute('data-stage-id'),{timeout:1200,intervals:[25,50,100]}).not.toBe(previous)}catch{break}}const stageId=await active.getAttribute('data-stage-id');if(!stageId||seen.has(stageId))break;seen.add(stageId);previous=stageId;const narrationId=`lesson-108-stage-${stageId}`;await expect.poll(()=>payloads.get(narrationId)??'',{timeout:5000,intervals:[50,100,200]}).not.toBe('');const text=payloads.get(narrationId)??'';const problems=speechProblems(text);if(problems.length)violations.push(`${stageId}: ${problems.join(', ')} :: ${text}`)}
  expect(seen.size).toBe(11);expect(violations.join('\n')).toBe('');
});
