...
async function routeNarrationAudit(page:Page,audit:RequestAudit){
  ...
  await page.route('**/api/narration',async route=>{
    const payload=route.request().postDataJSON() as {id?:string};if(payload.id)audit.ids.push(payload.id);audit.active+=1;audit.maxActive=Math.max(audit.maxActive,audit.active);
    await new Promise(resolve=>setTimeout(resolve,180));audit.active-=1;await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-six-task-five'});
  });
}
...
async function prepareTaskFive(page:Page){
  await installAudioMock(page);await page.addInitScript(({results})=>{
    localStorage.setItem('mathnikita-selected-lesson','6');localStorage.setItem('mathnikita-lesson-6-progress-v2',JSON.stringify({version:2,stageIndex:23,responses:{},orders:{},checked:{},results}));
    localStorage.setItem('mathnikita:lesson-6-revision-v2-migrated','1');localStorage.setItem('mathnikita:lesson-6-practice-v3-migrated','1');localStorage.setItem('mathnikita:extended-practice:6:v3','4');
  },{results:mainResults});
}
async function openTaskFive(page:Page,audit:RequestAudit){
  await prepareTaskFive(page);await routeNarrationAudit(page,audit);await page.goto('/',{waitUntil:'domcontentloaded'});await page.getByRole('button',{name:/Открыть урок 6:/}).click();await page.locator('.lesson-opening-start').click();await expect(page.locator('[data-practice-task="l6-p5"]')).toBeVisible({timeout:8_000});
}
...
test('lesson 6 practice sends only learner-visible narration before mentor interaction',async({page})=>{
  ...
  expect(audit.ids.some(id=>id.startsWith('mentor-practice-6-l6-p5-'))).toBeFalsy();
  expect(audit.maxActive).toBeLessThanOrEqual(2);
});

test('lesson 6 core CatMentor performs no speculative TTS generation',async({page})=>{
  ...
  await expect.poll(()=>audit.ids.filter(item=>item==='lesson-06-stage-l6-story').length,{timeout:8_000}).toBe(1);await page.waitForTimeout(500);
  expect(audit.ids.some(id=>id.startsWith('mentor-l6-intro-'))).toBeFalsy();expect(audit.maxActive).toBeLessThanOrEqual(2);
});
...
test('lesson 6 mentor distinguishes browser autoplay block from Sulafat outage',async({page})=>{
  ...
  // Keep this diagnostic independent from the task's own auto narration. The
  // voice-priority race is covered by the dedicated interruption suite.
