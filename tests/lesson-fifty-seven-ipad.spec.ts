...
async function openLesson57(page:Page){
  ...
  const lesson57=page.getByRole('button',{name:/Открыть урок 57:/});const lesson58=page.locator('.course-lesson-grid > button').nth(57);const lesson59=page.locator('.course-lesson-grid > button').nth(58);
  await expect(lesson57).toBeEnabled();await expect(lesson57).toHaveClass(/is-interactive/);await expect(lesson58).toBeEnabled();await expect(lesson59).toBeDisabled();
