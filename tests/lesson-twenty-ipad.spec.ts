...
async function openControl(page:Page){
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(21);
  await expect(page.locator('.course-lesson-grid > button.is-control-ready')).toHaveCount(1);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(22);
...
test('lesson 20 completes the full control work and reveals answers only after submission',async({page})=>{
  ...
  await expect(page.locator('[data-stage-id="l20-summary"]')).toBeVisible();await expect(page.locator('.control-score')).toContainText('14/14');await expect(page.locator('.control-score')).toContainText('Оценка: 5');await expect(page.locator('.control-score')).toContainText('Завершён');await expect(page.locator('.control-review-list section.correct')).toHaveCount(14);await expect(page.locator('.control-review-list section.wrong')).toHaveCount(0);
