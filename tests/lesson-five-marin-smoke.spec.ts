...
test('lesson 5 opening and first stage prefetch unified AI voice on iPad WebKit',async({page})=>{
  ...
  console.log('[lesson5-voice] navigate');
  await page.goto('/',{waitUntil:'domcontentloaded',timeout:10_000});
  await domClick(page.getByRole('button',{name:/Открыть урок 5:/}));

  console.log('[lesson5-voice] opening prefetch');
  ...
  console.log('[lesson5-voice] first stage prefetch');
  ...
  const audit=await page.evaluate(()=>(window as unknown as {__lessonFiveVoiceAudit:{systemSpeech:number}}).__lessonFiveVoiceAudit);
