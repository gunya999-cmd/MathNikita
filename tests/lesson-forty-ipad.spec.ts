...
async function openLesson40(page:Page){
  ...
  const lesson44=page.locator('.course-lesson-grid > button').nth(43);
  await expect(lesson40).toBeVisible();await expect(lesson40).toBeEnabled();await expect(lesson41).toBeEnabled();await expect(lesson42).toBeEnabled();await expect(lesson43).toBeEnabled();await expect(lesson44).toBeDisabled();
...
test('lesson 40 keeps protractor installation, persistence, angle builder and mandatory practice stable on iPad',async({page})=>{
  ...
  const collapsedMentor=page.locator('.cat-mentor-collapsed');await expect(collapsedMentor).toBeVisible();await collapsedMentor.click();await expect(page.locator('.cat-mentor-panel')).toBeVisible();await expect(page.locator('.cat-mentor-actions button')).toHaveCount(4);
