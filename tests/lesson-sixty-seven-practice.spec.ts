...
async function openSummary(page:Page){
  ...
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:67,stageIndex:35}})));
...
test('lesson 67 completes all 20 mandatory tasks / exactly 50 responses and persists completion',async({page})=>{
  ...
  const storageKey=extendedPracticeStorageKey(67);
  ...
  await finalStep.locator('textarea').fill('Я предсказываю изменение частного, объясняю роль деления и проверяю решение обратным действием.');
  ...
  const completion=await page.evaluate(()=>localStorage.getItem('mathnikita:lesson-complete:67'));expect(completion).toBeTruthy();
});
