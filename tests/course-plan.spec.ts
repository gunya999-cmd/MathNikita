...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(16);
  ...
  await expect(lessons.nth(15)).toBeEnabled();
  await expect(lessons.nth(16)).toBeDisabled();
  await expect(page.locator('body')).not.toContainText('Открытие темы');
  await expect(page.getByText('Полностью готовы первые шестнадцать интерактивных уроков.')).toBeVisible();
});
