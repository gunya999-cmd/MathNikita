...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 107',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(107);
  ...
  await expect(lessons.nth(106)).toBeEnabled();await expect(lessons.nth(106)).toHaveClass(/is-interactive/);
  ...
  await expect(page.getByText('Полностью готовы 107 уроков.')).toBeVisible();
});
