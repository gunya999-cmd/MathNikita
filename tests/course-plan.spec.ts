...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 102',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(97);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(102);
  ...
  for(let lessonNumber=91;lessonNumber<=102;lessonNumber+=1){
    const button=lessons.nth(lessonNumber-1);
    await expect(button).toBeEnabled();
  ...
  await expect(lessons.nth(101)).toBeEnabled();
  await expect(lessons.nth(101)).toHaveClass(/is-interactive/);

  await expect(lessons.nth(102)).toContainText('Смешанные числа');
  await expect(lessons.nth(102)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 102 уроков.')).toBeVisible();
});
