...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 71',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(68);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(71);
  ...
  for(let index=53;index<=70;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(71)).toContainText('Степень числа');
  await expect(lessons.nth(71)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 71 урок.')).toBeVisible();
});
