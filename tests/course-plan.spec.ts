...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 111',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(105);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(111);
  ...
  await expect(lessons.nth(111)).toBeDisabled();
  await expect(lessons.nth(174)).toContainText('Итоговая контрольная работа');
  await expect(page.getByText('Полностью готовы 111 уроков.')).toBeVisible();
});
