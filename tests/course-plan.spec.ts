...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 44',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(42);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(44);
  ...
  for(let index=33;index<=43;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(44)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 44 урока.')).toBeVisible();
});
