...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 101',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(96);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(101);
  ...
  for(let lessonNumber=91;lessonNumber<=101;lessonNumber+=1){
    const button=lessons.nth(lessonNumber-1);
    await expect(button).toBeEnabled();
  ...
  await expect(lessons.nth(100)).toBeEnabled();
  await expect(lessons.nth(100)).toHaveClass(/is-interactive/);

  await expect(lessons.nth(101)).toContainText('Смешанные числа');
  await expect(lessons.nth(101)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 101 уроков.')).toBeVisible();
});
