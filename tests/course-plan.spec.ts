...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 49',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(47);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(49);
  ...
  for(let index=33;index<=48;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(49)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 49 уроков.')).toBeVisible();
});
