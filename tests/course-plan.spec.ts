...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 67',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(64);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(67);
  ...
  for(let index=53;index<=66;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(67)).toContainText('Деление с остатком');
  ...
  await expect(page.getByText('Полностью готовы 67 уроков.')).toBeVisible();
});
