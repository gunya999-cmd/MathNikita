...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 104',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(99);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(104);
  ...
  for(let lessonNumber=91;lessonNumber<=104;lessonNumber+=1){const button=lessons.nth(lessonNumber-1);await expect(button).toBeEnabled();await expect(button).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(103)).toBeEnabled();await expect(lessons.nth(103)).toHaveClass(/is-interactive/);
  await expect(lessons.nth(104)).toContainText('Смешанные числа');await expect(lessons.nth(104)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 104 уроков.')).toBeVisible();
});
