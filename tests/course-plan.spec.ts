...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 66',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(63);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(66);
  ...
  for(let index=53;index<=65;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(page.getByText('Полностью готовы 66 уроков.')).toBeVisible();
});
