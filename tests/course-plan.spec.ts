...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 37',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(35);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(37);
  ...
  await expect(lessons.nth(37)).toBeDisabled();
