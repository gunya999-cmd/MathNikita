...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 55',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(52);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(55);
  ...
  await expect(lessons.nth(55)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 55 уроков.')).toBeVisible();
});
