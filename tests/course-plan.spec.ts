...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(11);
  ...
  await expect(page.getByText('Полностью готовы первые одиннадцать интерактивных уроков.')).toBeVisible();
});
