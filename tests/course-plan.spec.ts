...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(12);
  ...
  await expect(page.getByText('Полностью готовы первые двенадцать интерактивных уроков.')).toBeVisible();
});
