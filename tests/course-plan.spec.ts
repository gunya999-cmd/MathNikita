...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 78',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(74);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(78);
  ...
  await expect(lessons.nth(78)).toContainText('Прямоугольный параллелепипед. Пирамида');
  await expect(lessons.nth(78)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 78 уроков.')).toBeVisible();
});
