...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 36',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(34);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(36);
  ...
  await expect(lessons.nth(36)).toBeDisabled();
