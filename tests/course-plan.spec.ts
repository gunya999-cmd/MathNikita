...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 48',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(46);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(48);
  ...
  for(let index=33;index<=47;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(48)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 48 уроков.')).toBeVisible();
});
