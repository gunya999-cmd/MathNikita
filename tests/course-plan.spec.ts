...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 35',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(33);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(35);
  ...
  await expect(page.getByText('Полностью готовы 35 интерактивных уроков.')).toBeVisible();
});
