...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 77',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(73);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(77);
  ...
  await expect(lessons.nth(77)).toContainText('Прямоугольный параллелепипед. Пирамида');
  await expect(lessons.nth(77)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 77 уроков.')).toBeVisible();
});
