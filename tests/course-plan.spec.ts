...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 59',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(56);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(59);
  ...
  await expect(lessons.nth(59)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 59 уроков.')).toBeVisible();
});
