...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 64',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(61);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(64);
  ...
  for(let index=53;index<=63;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(page.getByText('Полностью готовы 64 урока.')).toBeVisible();
});