...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(14);
  ...
  await expect(page.getByText('Полностью готовы первые четырнадцать интерактивных уроков.')).toBeVisible();
});
