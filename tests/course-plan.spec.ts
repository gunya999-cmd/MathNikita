...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 56',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(53);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(56);
  ...
  await expect(lessons.nth(56)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 56 уроков.')).toBeVisible();
});
