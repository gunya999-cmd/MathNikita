...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 31',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(30);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(31);
  ...
  await expect(lessons.nth(30)).toBeEnabled();
  await expect(lessons.nth(31)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готов 31 интерактивный урок.')).toBeVisible();
});
