...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(10);
  ...
  await expect(page.getByText('Полностью готовы первые десять интерактивных уроков.')).toBeVisible();
});