...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 57',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(54);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(57);
  ...
  await expect(lessons.nth(57)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 57 уроков.')).toBeVisible();
});
