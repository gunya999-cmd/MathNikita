...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 58',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(55);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(58);
  ...
  await expect(lessons.nth(58)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 58 уроков.')).toBeVisible();
});
