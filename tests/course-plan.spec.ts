...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 65',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(62);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(65);
  ...
  for(let index=53;index<=64;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(65)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 65 уроков.')).toBeVisible();
});
