...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 70',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(67);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(70);
  ...
  for(let index=53;index<=69;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(69)).toBeEnabled();
  await expect(lessons.nth(70)).toContainText('Степень числа');
  await expect(lessons.nth(70)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 70 уроков.')).toBeVisible();
});
