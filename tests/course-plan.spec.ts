...
test('catalog follows the official 175-lesson Merzlyak plan', async ({ page }) => {
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(17);
  ...
  await expect(lessons.nth(16)).toBeEnabled();
  await expect(lessons.nth(17)).toBeDisabled();
  await expect(page.locator('body')).not.toContainText('Открытие темы');
  await expect(page.getByText('Полностью готовы первые семнадцать интерактивных уроков.')).toBeVisible();
});
