...
test('catalog follows the official 175-lesson Merzlyak plan through lesson 45',async({page})=>{
  await page.goto('/');
  ...
  await expect(page.locator('.course-lesson-grid > button.is-interactive')).toHaveCount(43);
  await expect(page.locator('.course-lesson-grid > button:not([disabled])')).toHaveCount(45);
  ...
  for(let index=33;index<=44;index+=1){await expect(lessons.nth(index)).toBeEnabled();await expect(lessons.nth(index)).toHaveClass(/is-interactive/)}
  ...
  await expect(lessons.nth(45)).toBeDisabled();
  ...
  await expect(page.getByText('Полностью готовы 45 уроков.')).toBeVisible();
});
