...
async function jump(page:import('@playwright/test').Page,stageIndex:number){await page.evaluate(index=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:107,stageIndex:index}})),stageIndex)}

test('lesson 107 sends clean Russian opening and all 29 stage payloads to Sulafat',async({page})=>{
  ...
  await page.route('**/api/narration',async route=>{const payload=(route.request().postDataJSON()??{}) as Payload;if(payload.id&&payload.text)payloads.set(payload.id,payload.text);await route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson-107-audit'})});
  await page.goto('/');await page.locator('button[aria-label^="Открыть урок 107:"]').evaluate((node:HTMLButtonElement)=>node.click());await page.locator('.voice-narrator > button').first().click();
  await expect.poll(()=>payloads.get('lesson-107-opening')??'').not.toBe('');const opening=payloads.get('lesson-107-opening')??'';expect(speechProblems(opening),opening).toEqual([]);
