...
const answers:Record<string,Answer> = {
  'l7-unit-equal': { type:'choice', value:'AB и NP' },
  'l7-task59': { type:'input', value:'136' },
  'l7-task61': { type:'input', value:'3' },
  'l7-practice1': { type:'choice', value:'1' },
  'l7-practice2': { type:'input', value:'6' },
  'l7-practice3': { type:'input', value:'7' },
  'l7-practice4': { type:'input', value:'5' },
  'l7-practice5': { type:'choice', value:'504 мм' },
  'l7-practice6': { type:'order', values:['Отметить точку P','Совместить нулевую отметку линейки с P','На отметке 7 см поставить точку K','Провести отрезок PK по линейке'] },
  'l7-extra-data': { type:'choice', value:'AC = 15 см' },
  'l7-quiz1': { type:'input', value:'40' },
  'l7-quiz2': { type:'input', value:'5' },
  'l7-quiz3': { type:'choice', value:'54 мм' },
  'l7-quiz4': { type:'choice', value:'3' },
  'l7-quiz5': { type:'input', value:'7' },
  'l7-challenge': { type:'input', value:'10' },
};
...
async function openLessonSeven(page:Page){
  ...
  await lessons.nth(6).click();
  await expect(page.getByRole('heading',{name:'Отрезок. Длина отрезка — закрепление'}).first()).toBeVisible();
...
test('lesson 7 completes every exercise and produces full scores on iPad WebKit',async({page})=>{
  ...
  await expect(page.locator('[data-stage-id="l7-summary"]')).toBeVisible();
...
test('lesson 7 keeps an answer after direct page navigation',async({page})=>{
  await openLessonSeven(page);

  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:7,stageIndex:7}})));
  const stage=page.locator('[data-stage-id="l7-practice1"]');
  await expect(stage).toBeVisible();
  await stage.getByRole('button',{name:'1',exact:true}).click();

  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:7,stageIndex:8}})));
  await expect(page.locator('[data-stage-id="l7-practice2"]')).toBeVisible();
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:7,stageIndex:7}})));

  await expect(page.locator('[data-stage-id="l7-practice1"] .choice-grid button.selected')).toHaveText('1');
});
