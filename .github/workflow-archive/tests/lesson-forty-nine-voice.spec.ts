...
async function installVoiceAudit(page:Page){
  await page.addInitScript(()=>{
    ...
    URL.createObjectURL=(blob:Blob|MediaSource)=>{
      if(blob instanceof Blob){const id=blobIds.get(blob);if(id)return`blob:l49-voice/${encodeURIComponent(id)}`}
    ...
    class MockAudio{
      ...
      id(){const prefix='blob:l49-voice/';return this.src.startsWith(prefix)?decodeURIComponent(this.src.slice(prefix.length)):this.src}
    ...
    (window as unknown as {__l49Voice:AuditState}).__l49Voice=audit;
...
async function events(page:Page){return page.evaluate(()=>(window as unknown as {__l49Voice:AuditState}).__l49Voice.events)}

test('lesson 49 immediately stops stale Sulafat and narrates the newly selected stage',async({page})=>{
  ...
  await page.route('**/api/narration',route=>route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson49'}));
  await page.goto('/');
  ...
  await expect(stage).toHaveAttribute('data-stage-id','l49-mission');
  const prefix='lesson-49-stage-';
  ...
  await page.evaluate(()=>{(window as unknown as {__l49Voice:AuditState}).__l49Voice.events=[]});
  ...
  await expect(stage).toHaveAttribute('data-stage-id','l49-quad-recall');
