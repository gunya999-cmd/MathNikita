...
test('lesson 6 practice auto-narrates the task without speculative mentor TTS',async({page})=>{
...
test('lesson 6 core CatMentor stays silent until a manual action and respects Sulafat request limit',async({page})=>{
  ...
  expect(audit.ids.some(id=>id.startsWith('mentor-l6-intro-'))).toBeFalsy();
  expect(audit.maxActive).toBeLessThanOrEqual(2);
});
...
test('lesson 6 mentor distinguishes browser autoplay block from Sulafat outage',async({page})=>{
  ...
  await expect.poll(()=>audit.ids.filter(id=>id==='lesson-06-practice-l6-p5').length,{timeout:4_000}).toBe(1);await page.waitForTimeout(500);
  expect(audit.ids.filter(id=>id==='mentor-practice-6-l6-p5-hint').length).toBe(0);
