...
test('lesson 5 has a varied full-paragraph mastery workload',()=>{
  const practice=extendedPracticeByLesson[5];
  expect(practice.tasks).toHaveLength(20);
  expect(extendedPracticeSetResponseCount(practice)).toBe(50);
  expect(practice.tasks.filter(task=>task.type==='multi-input')).toHaveLength(10);
...
test('generated mastery tasks are topic-specific across the course',()=>{
  ...
  expect(extendedPracticeByLesson[5].tasks.at(-1)?.prompt).toContain('разрядную границу');
...
test('practice checking accepts formatted, unit and multi-step answers',()=>{
  ...
  expect(isExtendedPracticeAnswerCorrect(extendedPracticeByLesson[5].tasks[10],{
    classes:'3',
    'hundred-thousands':'3',
    'five-value':'5 000',
    'full-thousands':'8 305',
  })).toBe(true);
  expect(isExtendedPracticeAnswerCorrect(extendedPracticeByLesson[5].tasks[14],{
    'after-99':'1 005',
    'three-digit-pages':'335',
    'last-page':'434',
    check:'1 194',
  })).toBe(true);
