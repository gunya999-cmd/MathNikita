...
async function openSummary(page:Page){
  ...
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:66,stageIndex:35}})));
...
test('lesson 66 completes all 20 mandatory tasks / exactly 50 responses and persists completion',async({page})=>{
  ...
  const practice=extendedPracticeByLesson[66];
  ...
  const storageKey=extendedPracticeStorageKey(66);
  ...
  await finalStep.locator('textarea').fill('Я объясняю роль деления, правило неизвестного компонента и проверяю ответ по исходному условию.');
  ...
  const completion=await page.evaluate(()=>localStorage.getItem('mathnikita:lesson-complete:66'));expect(completion).toBeTruthy();
});
