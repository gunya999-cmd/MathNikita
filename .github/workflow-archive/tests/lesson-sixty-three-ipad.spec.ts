...
async function jump(page:Page,stageIndex:number){await page.evaluate(stageIndex=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:63,stageIndex}})),stageIndex)}
...
test('lesson 63 is usable on iPad, preserves retry state and reaches 50-response practice',async({page})=>{
  ...
  await expect(stage).toHaveAttribute('data-stage-id','l63-mission');
  await expect(stage.locator('.lesson-controls')).toContainText('Этап 1 из 36');
  expect(await noHorizontalOverflow(page)).toBeTruthy();

  await jump(page,3);
  await expect(stage).toHaveAttribute('data-stage-id','l63-454-total');
  const input=stage.locator('.inline-answer input');
  for(const wrong of ['96','100']){await input.fill(wrong);await stage.locator('.check-button').click();await expect(stage.locator('.instant-feedback.bad')).toBeVisible()}
  await expect(stage.locator('.lesson-controls .primary')).toBeEnabled();
  await input.fill('98');await stage.locator('.check-button').click();await expect(stage.locator('.instant-feedback.good')).toBeVisible();
  const beforeReload=await page.evaluate(()=>JSON.parse(localStorage.getItem('mathnikita-lesson-63-progress-v1')??'null'));
  expect(beforeReload?.results?.['l63-p2']).toBe(true);

  await page.reload();
  await expect(page.getByRole('button',{name:/Открыть урок 63:/})).toBeVisible();
  await page.getByRole('button',{name:/Открыть урок 63:/}).click();await page.locator('.lesson-opening-start').click();
  await expect(page.locator('.lesson-runtime:not([hidden]) .interactive-stage')).toHaveAttribute('data-stage-id','l63-454-total');

  await jump(page,35);
  await expect(page.locator('.lesson-runtime:not([hidden]) .interactive-stage')).toHaveAttribute('data-stage-id','l63-summary');
