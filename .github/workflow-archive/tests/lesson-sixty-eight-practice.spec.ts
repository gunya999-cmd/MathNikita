...
async function openSummary(page:Page){
  ...
  await page.evaluate(()=>window.dispatchEvent(new CustomEvent('mathnikita-go-to-stage',{detail:{lessonNumber:68,stageIndex:35}})));
...
async function solveTask(page:Page,task:ExtendedPracticeTask){
  if(task.type==='choice')await page.locator('.extended-practice-options').getByRole('button',{name:task.answer,exact:true}).click();
    ...
    const inputs=page.locator('.extended-practice-multi input');await expect(inputs).toHaveCount(task.fields.length);
...
test('lesson 68 completes all 20 mandatory tasks / exactly 50 responses and persists completion',async({page})=>{
  test.setTimeout(180_000);
  await page.addInitScript(()=>{localStorage.setItem('mathnikita-mentor-auto-guide','false');localStorage.setItem('mathnikita-voice-settings-v4',JSON.stringify({engine:'browser',rate:.94}))});
  const practice=extendedPracticeByLesson[68];
  expect(practice.tasks).toHaveLength(20);expect(extendedPracticeSetResponseCount(practice)).toBe(50);
  await openSummary(page);
  await expect(page.locator('.extended-practice')).toHaveAttribute('data-practice-response-count','50');
  const storageKey=extendedPracticeStorageKey(68);
  for(let index=0;index<practice.tasks.length;index+=1){
    const task=practice.tasks[index];
    await expect(page.locator('.extended-practice[data-practice-task]')).toHaveAttribute('data-practice-task',task.id,{timeout:8_000});
    ...
    if(index===0){
      await page.reload({waitUntil:'domcontentloaded'});await openSummary(page);
  ...
  await expect(page.locator('.extended-practice.is-finished')).toContainText('Решены все 20 заданий');
  ...
  await finalStep.locator('textarea').fill('Я нахожу неполное частное, вычисляю остаток, проверяю r меньше делителя и восстанавливаю делимое по формуле.');
  ...
  const completion=await page.evaluate(()=>localStorage.getItem('mathnikita:lesson-complete:68'));expect(completion).toBeTruthy();
});
