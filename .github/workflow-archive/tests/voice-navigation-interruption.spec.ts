...
test('lesson 33 control auto-switches Sulafat without exposing tutoring feedback',async({page})=>{
  test.setTimeout(90_000);
  await installAudit(page);await routeNarration(page);await openLesson(page,33);
  const oldStage='l33-rules';const nextStage='l33-task1';
  const oldId='lesson-33-stage-l33-rules';const nextId='lesson-33-stage-l33-task1';
  await expect(page.locator('.lesson-runtime:not([hidden]) .interactive-stage[data-stage-id]')).toHaveAttribute('data-stage-id',oldStage);
