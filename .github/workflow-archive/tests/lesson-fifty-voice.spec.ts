...
async function installVoiceAudit(page:Page){
  await page.addInitScript(()=>{
    ...
    URL.createObjectURL=(blob:Blob|MediaSource)=>{
      if(blob instanceof Blob){const id=blobIds.get(blob);if(id)return`blob:l50-voice/${encodeURIComponent(id)}`}
      return nativeCreateObjectURL(blob);
    };
    class MockAudio{
      src='';preload='';playbackRate=1;currentTime=0;onended:(()=>void)|null=null;onerror:(()=>void)|null=null;private timer:number|null=null;private started=false;
      constructor(source=''){this.src=source}
      id(){const prefix='blob:l50-voice/';return this.src.startsWith(prefix)?decodeURIComponent(this.src.slice(prefix.length)):this.src}
    ...
    (window as unknown as {__l50Voice:AuditState}).__l50Voice=audit;
...
async function events(page:Page){return page.evaluate(()=>(window as unknown as {__l50Voice:AuditState}).__l50Voice.events)}

test('lesson 50 immediately stops stale Sulafat and narrates the newly selected stage',async({page})=>{
  ...
  await page.route('**/api/narration',route=>route.fulfill({status:200,contentType:'audio/wav',body:'RIFF-lesson50'}));
  await page.goto('/');
  ...
  await expect(stage).toHaveAttribute('data-stage-id','l50-mission');
  const prefix='lesson-50-stage-';
  ...
  const oldId=before.filter(event=>event.kind==='play'&&event.id.startsWith(prefix)).at(-1)?.id;
  expect(oldId).toBeTruthy();
  await page.evaluate(()=>{(window as unknown as {__l50Voice:AuditState}).__l50Voice.events=[]});
  await page.locator('.lesson-controls .primary').dispatchEvent('click');
  const immediate=await events(page);
  expect(immediate.some(event=>event.kind==='pause'&&event.id===oldId)).toBeTruthy();
  await expect(stage).toHaveAttribute('data-stage-id','l50-half-perimeter-model');
