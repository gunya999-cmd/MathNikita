...
function cancelStaleStudioGeneration(){
  ...
  activeStudioControllers.clear();
  prefetchQueue.length=0;queuedPrefetchKeys.clear();
}

if(typeof window!=='undefined'){
  window.addEventListener('mathnikita-audio-request',event=>{
    // A new learner-visible voice context wins immediately. Abort generation
    // and retry timers belonging to the screen/task the learner just left.
...
function isDynamicLessonPrefetch(id:string){return /^lesson-\d+-(?:stage|practice)-/.test(id)}
...
export function prefetchStudioAudioUrl(id:string,text:string){
  if(!id||!text||isDynamicLessonPrefetch(id))return;
...
function waitWithSignal(ms:number,signal:AbortSignal){
  ...
  return new Promise<void>((resolve,reject)=>{
    ...
    const onAbort=()=>{window.clearTimeout(timer);cleanup();reject(abortError())};
...
async function acquireStudioNetworkSlot(signal:AbortSignal){
  ...
  await new Promise<void>((resolve,reject)=>{
    ...
    const waiter=()=>{
      ...
      if(signal.aborted){releaseStudioNetworkSlot();reject(abortError());return}
      resolve();
    };
    const onAbort=()=>{
      if(settled)return;settled=true;
      const index=studioSlotWaiters.indexOf(waiter);if(index>=0)studioSlotWaiters.splice(index,1);
      cleanup();reject(abortError());
    };
    signal.addEventListener('abort',onAbort,{once:true});studioSlotWaiters.push(waiter);
  });
}
...
async function withStudioNetworkSlot<T>(work:()=>Promise<T>,signal:AbortSignal):Promise<T>{await acquireStudioNetworkSlot(signal);try{return await work()}finally{releaseStudioNetworkSlot()}}
async function requestStudioAudio(id:string,prepared:string,signal:AbortSignal,attempt=0):Promise<Blob>{
  ...
  if(!response.ok){
    if(attempt<2&&RETRYABLE_STATUS.has(response.status)){await waitWithSignal(retryDelayMs(response,attempt),signal);return requestStudioAudio(id,prepared,signal,attempt+1)}
...
export async function getStudioAudioUrl(id:string,text:string,mentorForegroundOverride=false):Promise<string>{
  ...
  if(id.startsWith('mentor-')){
    ...
    if(!foreground){
      ...
      await wait(300);
      // Never let speculative Pythagoras compete with a learner-visible TTS
      // generation. It can warm only during a genuinely idle provider window.
      if(activeStudioControllers.size>0)throw new Error('Background mentor warmup deferred');
      const warmed=readyAudioUrlCache.get(key);if(warmed)return warmed;const pending=audioUrlCache.get(key);if(pending)return pending;
    }
  }
