...
type PracticeNarration={id:string;text:string};
...
function resolvePracticeNarration(root:HTMLElement,lessonNumber:number):PracticeNarration|null{
  const scope=visiblePractice(root);if(!scope)return null;const taskId=scope.dataset.practiceTask??'';const set=extendedPracticeByLesson[lessonNumber];if(!set)return null;const index=set.tasks.findIndex(task=>task.id===taskId);if(index<0)return null;const task=set.tasks[index];return{id:practiceNarrationId(lessonNumber,task),text:practiceNarrationText(task,index,set.tasks.length)};
}
...
function getNarrationId(root:HTMLElement|null,mode:VoiceNarratorProps['mode'],lessonNumber:number){
  ...
  if(!Number.isFinite(resolvedLesson)||resolvedLesson<1)return'';const lessonId=String(resolvedLesson).padStart(2,'0');if(mode==='opening')return`lesson-${lessonId}-opening`;if(!root)return'';
...
export function VoiceNarrator({rootRef,mode,lessonNumber,openingText}:VoiceNarratorProps){
  ...
  const sessionRef=useRef(0);const audioRef=useRef<HTMLAudioElement|null>(null);const narratorRef=useRef<HTMLDivElement|null>(null);
  ...
  function failStudio(session:number,message='AI-голос временно недоступен. Нажми ещё раз.'){if(sessionRef.current!==session)return;audioRef.current=null;setSpeaking(false);setStudioIssue(message)}
  ...
  function playStudioSource(source:string,session:number){
    ...
    const fail=()=>failStudio(session);audio.onended=()=>{if(sessionRef.current===session)setSpeaking(false);audioRef.current=null};audio.onerror=fail;void audio.play().catch(fail);
  }
  ...
  async function startStudioSpeech(text:string,narrationId:string,session:number){
    ...
    try{const source=await getStudioAudioUrl(narrationId,text);if(sessionRef.current===session)playStudioSource(source,session)}catch{failStudio(session)}
  }

  function speak(){
    if(speaking){stop();return}const text=getNarrationText(rootRef.current,mode,openingText,lessonNumber);if(!text)return;
    stop();setStudioIssue('');const session=sessionRef.current+1;sessionRef.current=session;window.dispatchEvent(new CustomEvent('mathnikita-audio-request',{detail:{source:'narrator'}}));const narrationId=getNarrationId(rootRef.current,mode,lessonNumber);
    if(engine==='studio'){const readySource=peekStudioAudioUrl(narrationId,text);if(readySource){playStudioSource(readySource,session);return}void startStudioSpeech(text,narrationId,session);return}
