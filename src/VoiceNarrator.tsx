...
export function VoiceNarrator({rootRef,mode,lessonNumber,openingText}:VoiceNarratorProps){
  ...
  useEffect(()=>{
    ...
    if(mode==='opening'){const id=getNarrationId(null,mode,lessonNumber);const text=getNarrationText(null,mode,openingText);if(id&&text)prefetchStudioAudioUrl(id,text);return}
    let retryTimer:number|null=null;let retries=0;
    const warm=()=>{const root=rootRef.current;const id=getNarrationId(root,mode,lessonNumber);const text=getNarrationText(root,mode,openingText);if(id&&text){prefetchStudioAudioUrl(id,text);retries=0;return}if(retries<6){retries+=1;retryTimer=window.setTimeout(warm,60*retries)}};
  ...
  function speak(){
    if(speaking){stop();return}const text=getNarrationText(rootRef.current,mode,openingText);if(!text)return;
