...
export function PracticePythagoras({lessonNumber,task,checkState,attempts}:Props){
  ...
  function speak(text=message,voiceAction=action){
    ...
    if(typeof Audio==='undefined'){setVoiceIssue(true);return}
    const id=`mentor-practice-${lessonNumber}-${safeToken(task.id)}-${safeToken(voiceAction)}`;setSpeaking(true);
    void getStudioAudioUrl(id,text).then(source=>{if(token!==tokenRef.current)return;const audio=new Audio(source);audioRef.current=audio;audio.preload='auto';audio.playbackRate=settings.rate;audio.onended=()=>{if(token===tokenRef.current)setSpeaking(false);audioRef.current=null};const fail=()=>{if(token===tokenRef.current){audioRef.current=null;setSpeaking(false);setVoiceIssue(true)}};audio.onerror=fail;void audio.play().catch(fail)}).catch(()=>{if(token===tokenRef.current){setSpeaking(false);setVoiceIssue(true)}});
  }
  ...
  useEffect(()=>{stop();setAction('status');setVoiceIssue(false)},[task.id]);
