...
function exampleGuide(task:ExtendedPracticeTask){const base=responseGuide(task);return `Возьми похожий более простой случай и выполни на нём тот же ход, не решая исходное задание наугад. ${base} Затем вернись к своим данным. ${task.hint}`}
...
function issueFromError(error:unknown):VoiceIssue{
  const name=typeof error==='object'&&error&&'name'in error?String((error as {name?:unknown}).name??''):'';if(name==='NotAllowedError')return'autoplay';
  const text=String(error??'');if(/\b429\b|rate limit|too many requests|\b503\b/i.test(text))return'busy';return'unavailable';
}

export function PracticePythagoras({lessonNumber,task,checkState,attempts}:Props){
  const[action,setAction]=useState<MentorAction>('status');const[speaking,setSpeaking]=useState(false);const[voiceIssue,setVoiceIssue]=useState<VoiceIssue>('');const audioRef=useRef<HTMLAudioElement|null>(null);const tokenRef=useRef(0);
  ...
  useEffect(()=>{
    stop();setAction('status');setVoiceIssue('');if(loadVoiceSettings().engine!=='studio')return;
  ...
  return <aside className={`practice-pythagoras is-${checkState}`} aria-label="Пифагор — наставник обязательной практики">
    ...
    <div className="practice-pythagoras-actions">
      <button type="button" onClick={()=>choose('different')}>↻ Объясни иначе</button><button type="button" onClick={()=>choose('example')}>▣ Дай пример</button><button type="button" onClick={()=>choose('hint')}>✦ Подсказка</button><button type="button" onClick={()=>choose('why')}>? Почему так?</button>
    </div>
  </aside>;
}
