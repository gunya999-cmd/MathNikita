...
export const lessonTwentyOneStages:Stage[]=[
  ...
  {id:'l21-practice2',kind:'practice',eyebrow:'Практика · 2/6',title:'Два переноса в одной сумме',body:'В этой сумме перенос понадобится дважды. Следи за каждым переносом и не теряй его.',activity:{id:'l21-p2',type:'input',prompt:'Вычисли: 38 476 + 5 209.',answer:'43685',placeholder:'Сумма',explanation:'6+9=15: пишем 5, переносим 1. Затем 7+0+1=8, 4+2=6, 8+5=13 и переносим 1 в десятки тысяч. Итог: 43 685.'}},
...
export function NaturalNumberAdditionPlayer(){
  ...
  function checkAnswer(){
    if(!activity)return;
    const correct=activity.type==='order'?sameOrder(currentOrder,activity.answer as string[]):semanticAnswersEquivalent(String(currentResponse),String(activity.answer as string));
