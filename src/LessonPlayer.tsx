...
const lessonOneStages: Stage[] = [
  { id:'story', kind:'story', eyebrow:'Начинаем с понятного', title:'Как ответить на вопрос «сколько?»', body:'Представь полку с книгами. Мы касаемся каждой книги один раз и называем числа по порядку: один, два, три, четыре, пять. Последнее названное число и показывает количество книг.', note:'Главная идея: один предмет — одно число. Нельзя пропускать предметы или считать один предмет дважды.' },
  { id:'count-rule', kind:'model', eyebrow:'Разбираемся шаг за шагом', title:'Почему последним получается именно 5', body:'У каждой книги есть своё место в счёте. Первая книга получает число 1, вторая — 2, и так далее. Когда книги закончились, последнее число стало ответом.', note:'Число 5 здесь не название книги. Оно показывает, сколько всего книг.' },
  { id:'natural-check', kind:'guided', eyebrow:'Проверяем понимание', title:'Какие числа называют натуральными', body:'Натуральные числа используют, когда считают предметы: 1, 2, 3, 4, 5… В школьном курсе натуральный ряд начинается с 1.', activity:{ id:'t1', type:'choice', prompt:'Какое число подходит для ответа «сколько котят в коробке?»', options:['три','синий','быстро','круглый'], answer:'три', explanation:'«Три» отвечает на вопрос «сколько?», поэтому это натуральное число.' } },
  { id:'model', kind:'model', eyebrow:'Интерактивная модель', title:'У каждого числа есть следующее', body:'Нажимай «+1». Ты увидишь, что после любого натурального числа можно получить ещё одно — большее на единицу.', note:'Поэтому самого большого натурального числа не существует: к любому числу можно прибавить 1.' },
  { id:'choice', kind:'guided', eyebrow:'Пробуем вместе', title:'Какое число идёт следующим?', body:'Следующее число всегда на 1 больше предыдущего.', activity:{ id:'a1', type:'choice', prompt:'После числа 39 идёт…', options:['38','40','49','30'], answer:'40', explanation:'39 + 1 = 40. Следующее число всегда на единицу больше.' } },
  { id:'compare', kind:'guided', eyebrow:'Пробуем вместе', title:'Сравниваем числа', body:'На числовом луче число, которое расположено правее, больше.', activity:{ id:'a2', type:'compare', left:'27', right:'32', answer:'<', explanation:'27 встречается раньше 32, поэтому 27 < 32.' } },
  ...
  { id:'challenge', kind:'challenge', eyebrow:'Задача со звёздочкой', title:'Можно ли назвать самое большое число?', body:'Попробуй объяснить словами. Что произойдёт, если к предложенному числу прибавить 1?', activity:{ id:'c1', type:'choice', prompt:'Какой вывод правильный?', options:['Самое большое число — 1 000 000','Самого большого натурального числа нет','Самое большое число — 999 999','Самое большое число зависит от учебника'], answer:'Самого большого натурального числа нет', explanation:'Какое бы число ни назвали, можно прибавить 1 и получить ещё большее.' } },
  { id:'summary', kind:'summary', eyebrow:'Итог', title:'Урок завершён', body:'Ты понял, как счёт связывает предметы и числа, умеешь продолжать натуральный ряд, находить предыдущее и следующее число и сравнивать числа.' },
];
...
export function LessonPlayer(){
  ...
  const visualModel=useMemo(()=>{
    if(stage?.id==='story') return <div className="theory-scene"><div className="pifagor-bubble"><b>Кот Пифагор</b><span>Давай посчитаем книги вместе. Следи: каждой книге — только одно число.</span></div><div className="object-count" aria-label="Пять книг">{['📘','📗','📙','📕','📓'].map((item,i)=><span key={i} style={{animationDelay:`${i*90}ms`}}>{item}<small>{i+1}</small></span>)}</div></div>;
    if(stage?.id==='count-rule') return <div className="count-explainer"><div><b>1</b><span>первая книга</span></div><i>→</i><div><b>2</b><span>вторая книга</span></div><i>→</i><div><b>3</b><span>третья книга</span></div><i>…</i><div className="final-count"><b>5</b><span>всего книг</span></div></div>;
    if(stage?.id==='natural-check') return <div className="concept-strip"><div><span>🐱🐱🐱</span><b>3 котёнка</b></div><div><span>✏️✏️✏️✏️</span><b>4 карандаша</b></div><div><span>⭐️⭐️⭐️⭐️⭐️</span><b>5 звёзд</b></div></div>;
  ...
  return <main className="lesson-player-page">
    ...
    {lessonNumber!==1?<GenericLesson lessonNumber={lessonNumber}/>:<section className="lesson-workspace interactive-workspace">
      <header className="lesson-header"><div><span>Урок 1 из 175 · Натуральные числа</span><h1>Натуральные числа и счёт</h1><p>Сначала разберём идею на простых примерах, затем потренируемся и пройдём мини-проверку.</p></div><div className="lesson-duration">25–30 мин</div></header>
      <div className="lesson-progress"><i style={{width:`${progress}%`}} /></div>
      <div className="stage-counter">Этап {stageIndex+1} из {lessonOneStages.length}</div>
      <article className={`interactive-stage stage-${stage.kind}`}><div className="stage-copy"><span>{stage.eyebrow}</span><h2>{stage.title}</h2><p>{stage.body}</p>{stage.note?<div className="theory-note"><b>Запомни</b><span>{stage.note}</span></div>:null}</div>{visualModel}{activity&&renderActivity(activity)}{checked&&activity?<div className={`instant-feedback ${correct?'good':'bad'}`}><b>{correct?'Верно!':'Пока не получилось'}</b><span>{correct?activity.explanation:'Посмотри на модель ещё раз. '+activity.explanation}</span></div>:null}{stage.kind==='quiz'&&checked?<div className="quiz-meter"><span>Текущий результат</span><b>{quizScore} из 4</b></div>:null}{stage.kind==='summary'?<div className="summary-card"><div><span>Мини-проверка</span><b>{quizScore}/4</b><small>{quizScore>=3?'Тема усвоена':'Нужно короткое повторение'}</small></div><div><span>Практика</span><b>{Object.keys(results).filter(id=>id.startsWith('a')).filter(id=>results[id]).length}/5</b><small>выполнено верно</small></div><div><span>Следующий шаг</span><b>Урок 2</b><small>Натуральный ряд и закономерности</small></div></div>:null}</article>
