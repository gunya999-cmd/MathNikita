...
const readyDescriptions:Record<number,string>={5:'Обобщение § 2: чтение, запись и разбор многозначных чисел, ведущий нуль и задачи на количество цифр.',6:'Новая тема § 3: точка и отрезок, измерение и построение длины, равные отрезки и свойство AB = AC + CB.'};
...
export function CourseCatalog({selectedLesson,onOpenLesson}:Props){
  ...
  return <main className="course-catalog-page">
    ...
    <section className="course-catalog-toolbar" aria-label="Поиск урока"><div><b>{totalLessons} уроков в официальном плане</b><span>Полностью готовы первые шесть интерактивных уроков. Остальные сохранены в точной последовательности учебного года.</span></div><label><span className="sr-only">Найти урок</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Найти тему, параграф или номер урока"/></label></section>
