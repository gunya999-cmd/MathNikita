...
export function CourseCatalog({selectedLesson,onOpenLesson}:Props){
  ...
  return <main className="course-catalog-page">
    ...
    <section className="course-catalog-toolbar" aria-label="Поиск урока"><div><b>{totalLessons} уроков в официальном плане</b><span>Полностью готовы первые семь интерактивных уроков. Остальные сохранены в точной последовательности учебного года.</span></div><label><span className="sr-only">Найти урок</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Найти тему, параграф или номер урока"/></label></section>
