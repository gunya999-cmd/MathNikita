    ...
    <div className="course-chapter-list">{groups.map((group,groupIndex)=>{
      ...
      return <details className="course-chapter-group" key={group.unit} open={containsSelected||groupIndex===0||Boolean(query.trim())}>
        ...
        <section className="course-lesson-grid" aria-label={group.unit}>{group.lessons.map(lesson=>{
          const ready=lesson.available;const selected=lesson.number===selectedLesson;const rich=richLessonByNumber.get(lesson.number);const description=ready?(rich?.goal??readyDescriptions[lesson.number]??planLabel(lesson)):planLabel(lesson);const stateClass=ready?'is-interactive':'is-locked';
