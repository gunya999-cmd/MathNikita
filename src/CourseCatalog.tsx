...
const readyDescriptions:Record<number,string>={5:'Обобщение § 2: чтение, запись и разбор многозначных чисел, ведущий нуль и задачи на количество цифр.',6:'Новая тема § 3: точка и отрезок, измерение и построение длины, равные отрезки и свойство AB = AC + CB.',7:'Закрепление § 3: измерение и построение, цепочки точек, перевод единиц и задачи № 59, 61, 63–66 и 75.',8:'Новая тема § 3: ломаная, её вершины и звенья, длина ломаной, замкнутые ломаные и задачи № 68, 71, 74, 77.',9:'Обобщение § 3: точка, отрезок, свойство длины, единицы, равные отрезки, ломаная и итоговый контроль темы.',10:'Новая тема § 4: плоскость, прямая, свойство прямой, обозначения прямых и луч с его началом.',11:'Закрепление § 4: геометрические построения, направления лучей, взаимное расположение фигур и задачи № 94, 95, 98, 99, 102.'};
...
export function CourseCatalog({selectedLesson,onOpenLesson}:Props){
  ...
  return <main className="course-catalog-page">
    ...
    <section className="course-catalog-toolbar" aria-label="Поиск урока"><div><b>{totalLessons} уроков в официальном плане</b><span>Полностью готовы первые одиннадцать интерактивных уроков. Остальные сохранены в точной последовательности учебного года.</span></div><label><span className="sr-only">Найти урок</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Найти тему, параграф или номер урока"/></label></section>
    <div className="course-chapter-list">{groups.map((group,groupIndex)=>{
      ...
      return <details className="course-chapter-group" key={group.unit} open={containsSelected||groupIndex===0||Boolean(query.trim())}>
        ...
        <section className="course-lesson-grid" aria-label={group.unit}>{group.lessons.map(lesson=>{
          const ready=lesson.available;const selected=lesson.number===selectedLesson;const rich=richLessonByNumber.get(lesson.number);const description=ready?(rich?.goal??readyDescriptions[lesson.number]??planLabel(lesson)):planLabel(lesson);const stateClass=!ready?'is-locked':lesson.number===11?'is-ready':'is-interactive';
          return <button key={lesson.number} type="button" className={`${stateClass} ${selected?'is-selected':''} is-${lesson.lessonType}`} onClick={()=>ready&&onOpenLesson(lesson.number)} disabled={!ready} aria-label={ready?`Открыть урок ${lesson.number}: ${lesson.title}`:`Урок ${lesson.number} в разработке`}><span>{lesson.number}</span><div><small>{ready?'Готов к прохождению':planLabel(lesson)}</small><b>{lesson.title}</b><p>{description}</p></div><i aria-hidden="true">{ready?'→':lesson.lessonType==='control'||lesson.lessonType==='final'?'✓':'🔒'}</i></button>
        })}</section>
      </details>;
    })}</div>
