...
export function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {screen === 'map' && <main className="dashboard"><header><span>Карта знаний</span><h1>Курс математики 5 класса</h1><p>175 уроков по I варианту планирования Мерзляка.</p></header><div className="world-grid">{islands.map(([icon,title,count],i)=><article key={title}><i>{icon}</i><h3>{title}</h3><p>{count} уроков</p><b>{i===0?'20 уроков готовы':i===1?'7 уроков готовы':'В разработке'}</b></article>)}</div></main>}
